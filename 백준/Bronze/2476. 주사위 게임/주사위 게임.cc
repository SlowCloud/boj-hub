...
template <typename ...Args>
constexpr inline void write_debug(Args& ...args) {
	if constexpr (DEBUG) {
		(std::cout << ... << args);
	}
}
// #define debug if contexpr(DEBUG) cout
...
void solve() {
	...
	while (N--) {
		...
		if (a == b && b == c) {
			res = max(res, 10'000 + (a * 1'000));
		}
		else if (a != b && b != c && a != c) {
			res = max(res, max({ a,b,c }) * 100);
		}
		else {
			int now = a == b ? a : b == c ? b : a;
			res = max(res, 1'000 + now * 100);
		}
	}
