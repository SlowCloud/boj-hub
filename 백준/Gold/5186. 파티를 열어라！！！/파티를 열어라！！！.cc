...
int main() {
	...
	FOR(k,1,K + 1) {
		...
		memset(drunken, 0, sizeof drunken);
		memset(not_drunken, 0, sizeof not_drunken);
		...
		FOR(i,0,n) {
			int n; char c;
			read(n, c);
			(c == 'I' ? drunken : not_drunken)[n]++;
		}

		vector<pii> cars;
		FOR(i, 0, c) {
			int a, b; cin >> a >> b;
			cars.push_back({ a,b }); // destination, capability
		}

		sort(ALL(cars), [](pii a, pii b) {
			if (a.first != b.first) return a.first < b.first;
			return a.second > b.second;
		});

		FOR(t, 0, c) {
			auto [dest, capa] = cars[t];

			if (not_drunken[dest] == 0) continue;

			not_drunken[dest]--;
			capa--;

			if (drunken[dest]) {
				int tmp = min(drunken[dest], capa);
				drunken[dest] -= tmp;
				capa -= tmp;
			}

			if (not_drunken[dest]) {
				int tmp = min(not_drunken[dest], capa);
				not_drunken[dest] -= tmp;
				capa -= tmp;
			}

		}

		int res = accumulate(drunken, drunken + 555, 0) + accumulate(not_drunken, not_drunken + 555, 0);

		std::cout << "Data Set " << k << ": " << res << endl;
	}
